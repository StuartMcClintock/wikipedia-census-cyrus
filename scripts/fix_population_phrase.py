#!/usr/bin/env python3
"""
Scan municipality articles (from FIPS mappings) for the phrase:
...
POP_PHRASE_RE = re.compile(
    ...
    r"\s+families\s+(?P<tail>residing\s+in\s+the\s+city)",
...
def main() -> None:
    ...
    for name in load_municipality_titles(args.state_postal, args.municipality_type):
        ...
        try:
            ...
            new_text, count = replace_population_phrase(wikitext)
            if count == 0:
