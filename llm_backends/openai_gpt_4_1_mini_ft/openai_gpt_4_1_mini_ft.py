"""
Backend for the fine-tuned gpt-4.1-mini model trained on Iowa demographics
section rewrites (see fine_tuning/ for the training corpus and pipeline).

Same shape as openai_gpt_5_mini.py — same prompts, same function signatures —
but with FT-specific tweaks:
  * temperature=0 for deterministic output (FT model isn't restricted to 1)
  * no service_tier="flex" (not supported on fine-tuned models)
  * MODEL_NAME defaults to the FT id, but ACTIVE_MODEL still wins so you can
    A/B test other FT versions without code changes.
"""
...
MODEL_NAME = "ft:gpt-4.1-mini-2025-04-14:personal:wiki-census-iowa:DbNjEIZz"
...
def _chat_complete(prompt: str, *, max_tokens: int = 25000) -> str:
    ...
    resp = client.chat.completions.create(
        model=resolve_model_alias(os.getenv("ACTIVE_MODEL", MODEL_NAME)),
        messages=[
...
def update_demographics_section(
    ...
    # Prompt is byte-for-byte the same as what the FT model was trained on
    # (see fine_tuning/prepare_sft_dataset.py:USER_PROMPT_TEMPLATE).
    prompt = f"""
...
def update_lede(current_lede_text: str, population_sentence: str, suppress_out: bool = True) -> str:
    # The FT model wasn't trained on this task, so behavior here is whatever
    # base gpt-4.1-mini does. Kept for API parity with the other backends.
    prompt = f"""
